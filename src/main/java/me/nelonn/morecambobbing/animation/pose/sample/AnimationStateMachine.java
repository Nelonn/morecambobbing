...
public class AnimationStateMachine<S extends Enum<S>> extends TimeBasedAnimationState {
    ...
    private StateTransition getStateTransition(Enum<S> origin, Enum<S> destination) {
         ...
         */
        MoreCamBobbingMain.LOGGER.error("Could not get state transition origin {} destination {} in state machine {}", origin, destination, this.getIdentifier());
        return null;
    }
    ...
    public <L extends Enum<L>> AnimationStateMachine<S> setPose(S identifier, AnimationPose<L> animationPose) {
        if (this.statesHashMap.containsKey(identifier)) {
            ...
            MoreCamBobbingMain.LOGGER.error("Tried setting pose to invalid state {} in state machine {}", identifier, this.getIdentifier());
        }
        return this;
    }
    ...
    public <L extends Enum<L>> AnimationPose<L> sample(LocatorSkeleton<L> locatorSkeleton, AnimationDataContainer.CachedPoseContainer cachedPoseContainer) {
        if (this.activeStates.size() > 0) {
            ...
            //MoreCamBobbingMain.LOGGER.info(this.activeStates.toString());
            return animationPose;
        }
        MoreCamBobbingMain.LOGGER.warn("No active states in state machine {}", this.getIdentifier());
    ...
    public void tick() {
        ...
        if (this.statesHashMap.size() == 0 || this.activeStates.size() == 0) {
            MoreCamBobbingMain.LOGGER.warn("State machine {} not evaluated due to no active states", this.getIdentifier());
            return;
        }
